...
extern "C" {
  void add_c( uint64_t         i_n_values,  
              ...
              uint64_t const * i_b,         
              uint64_t       * o_c );       
  void add_asm( uint64_t         i_n_values,  //x0
                uint64_t const * i_a,         //x1
                uint64_t const * i_b,         //x2
                uint64_t       * o_c );       //x3
}
...
int main() {
  ...
  add_c( l_n_values,l_a,l_b,l_c_0 );
  ...
  add_asm( l_n_values, l_a, l_b, l_c_1 );
...
/**
 * The two functions add_asm and add_c in the template shall perform the same task: (componentwise) add the values in the two arrays and, 
 * and store the result as the entry of a third array in memory, namely, add the 64-bit entries from the two arrays l_a and l_b, 
 * and save the results as the corresponding entries in the array l_c of length l_n_values.
 */